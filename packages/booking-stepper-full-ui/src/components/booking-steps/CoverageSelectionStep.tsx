...
}> = ({ vehicleClass, highlightedRegions, selectedRegions, onRegionClick }) => {
  const getRegionStyle = (regionId: string) => {
    ...
    if (isSelected) {
      ...
      if (regionId === 'windshield_full' && selectedRegions.includes('sun_brow')) {
        baseStyle = 'fill-primary/40 stroke-primary stroke-2';
      } else if (regionId === 'sun_brow' && selectedRegions.includes('windshield_full')) {
        baseStyle = 'fill-primary/60 stroke-primary stroke-2';
      } else {
        baseStyle = 'fill-primary/40 stroke-primary stroke-2';
      }
    } else if (isHighlighted) {
      baseStyle = 'fill-primary/20 stroke-primary/60 stroke-1';
    } else {
      baseStyle = 'fill-transparent stroke-muted-foreground/30 stroke-1';
    }

    // Add clickable styling
    return `${baseStyle} cursor-pointer hover:fill-primary/30 hover:stroke-primary/80 hover:stroke-2 transition-all duration-200 active:scale-95`;
  };
      ...
      <div className="space-y-4">
        ...
        <svg width="280" height="160" viewBox="0 0 280 160" className="max-w-full">
          ...
          <motion.path
            ...
            onClick={() => handleRegionClick('windshield_full')}
          />
          ...
          <motion.path
            ...
            animate={{ opacity: 1 }}
            transition={{ duration: 0.2 }}
            onClick={() => handleRegionClick('sun_brow')}
          />
          ...
          <motion.rect
            ...
            animate={{ opacity: 1 }}
            transition={{ duration: 0.2 }}
            onClick={() => handleRegionClick('front_sides')}
          />
          ...
          <motion.rect
            ...
            animate={{ opacity: 1 }}
            transition={{ duration: 0.2 }}
            onClick={() => handleRegionClick('rear_sides')}
          />
          ...
          <motion.path
            ...
            animate={{ opacity: 1 }}
            transition={{ duration: 0.2 }}
            onClick={() => handleRegionClick('rear_glass')}
          />
        </svg>
        ...
        <div className="text-sm space-y-2">
          ...
          <div className="flex items-center gap-2">
            <div className="w-3 h-3 border-2 border-dashed border-muted-foreground/50 rounded-sm cursor-pointer"></div>
            <span className="text-xs text-muted-foreground">Clickable</span>
          </div>
          <p className="text-xs text-muted-foreground font-medium">
            💡 Click directly on vehicle areas to quickly select coverage options.
          </p>
        </div>
      </div>
    </div>
...
export const CoverageSelectionStep: React.FC<CoverageSelectionStepProps> = ({
  ...
  // Map regions to coverage options for interactive clicking
  const handleRegionClick = (regionId: string) => {
    // Find which coverage option(s) include this region
    const matchingOption = COVERAGE_OPTIONS.find(option =>
      option.regions.includes(regionId)
    );

    if (matchingOption) {
      handleCoverageToggle(matchingOption.id);
    }
  };
            ...
            >
              ...
              <div className="space-y-2">
                ...
                <div className="flex justify-between items-center">
                  ...
                  <span className="font-semibold text-primary">{calculateDuration()}</span>
                </div>
